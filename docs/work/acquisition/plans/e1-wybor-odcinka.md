...
### Historia zgody Z-1 — zastąpione tempo

**Historyczne uzupełnienie Z-1 z 2026-09-25:** Torrentio na VPS zwróciło Cloudflare HTTP 403; dostęp lokalny na Windows działa. N-01 przeniesiono na Windows właściciela, nadal na 7 dni. Dla N-02/N-04 pierwotnie przyjęto lokalne archiwum `%USERPROFILE%/acquisition-corpus`, limit 1000 prób Torrentio w ruchomych 24 h i równomierny odstęp 86,4 s. Późniejsza decyzja poniżej zastąpiła to tempo i limit dobowy oraz autoryzowała konkretną migrację kontraktu. Historia prób VPS i lokalnych pozostaje zachowana. Jest to historia uzupełnień zgody, nie zmiana treści README.

### Historia zgody Z-1 — tempo adaptacyjne przed rezerwą

**Kolejna decyzja właściciela — 2026-09-25, tempo adaptacyjne i oznaczanie równoległe:** obowiązuje limit **3800 prób Torrentio w każdym ruchomym oknie 24 h**. Nadal obowiązują **4000 łącznie, 2000 tytułów oraz 4 doby od pierwotnego startu**, wspólne N-02/N-04, zachowanie wszystkich prób i odrębne 7 dni N-01. Start co 5 s; po każdych 50 kolejnych czystych odpowiedziach 200 z poprawnym JSON odstęp maleje o połowę, minimum 1 s. HTTP 429 podwaja odstęp i wymaga czekania według `Retry-After`/cooldownu `RequestControl`; trwała dolna granica odstępu nie może już spaść poniżej odstępu z chwili tego 429. Journal zapisuje odstęp przy zdarzeniu, numer próby oraz stan tempa i liczników 429/403, odtwarzany po restarcie. HTTP 403, strona „Attention Required” / „you have been blocked” lub challenge zatrzymują Torrentio natychmiast, bez automatycznego retry; inne źródła mogą dokończyć metadane bieżącego tytułu. Limit 5000/24 h/IP pochodzi z publicznego kodu Torrentio (`serverless.js:25–34`, research release-validation), nie z potwierdzonej konfiguracji produkcji; reguły Cloudflare pozostają nieznane.

**Usunięcie przyczyny lokalnego 403 — fakt i zgoda 2026-09-25:** domyślny `python-httpx/0.28.1` otrzymał 403, `AniShift/0.1` otrzymał 200 na komputerze właściciela. Każde HTTP zbieracza do Torrentio, AniList i ani.zip używa importowanej istniejącej stałej `anishift/services/torrents/nyaa.py:USER_AGENT`. Właściciel autoryzuje jednorazową, dopisywaną do journalu migrację kontraktu schema 2 → 3 (nowy SHA skryptu, tempo i limit 3800), z jawnym wskazaniem starego SHA; wszystkie pozostałe pola kontraktu, historia, surowe pliki, budżet i deadline pozostają zachowane. Osobne `-RetryBlocked` / `--retry-blocked` autoryzuje tylko obecne blokady Torrentio po diagnozie; nie obejmuje następnej blokady i nie kasuje wcześniejszego 403. Inny mismatch nadal wymaga decyzji. Ta zgoda jest konkretnym wyjątkiem od historycznego zakazu migracji powyżej.
...
**Obowiązująca decyzja właściciela — 2026-09-25, korpus główny i rezerwa.** Poniższe ustalenia zastępują wcześniejsze ograniczenia 2000 tytułów i 4000 prób łącznie oraz dawny warunek zakończenia po podstawowym doborze:

1. **`main` = niezmienione 2000 wpisów z `titles.json` + 50 dodatkowych długich serii.** Kryterium dodatkowej puli: TV/TV_SHORT/ONA z liczbą odcinków AniList ≥100. Dobór bez powtórzeń ID, seed **20260925**, pięć dotychczasowych przedziałów lat, przeplot przedziałów, popularność malejąco i losowanie remisów z ziarnem. Mniej niż 50 jest dopuszczalne wyłącznie jako jawny niedobór po wyczerpaniu paginowanego katalogu; nie jest zaliczeniem pokrycia numeracji absolutnej. Metadane ani.zip nadal stanowią dowód rzeczywistej numeracji. Manifest zapisuje liczby dostępnych, wybranych i pominiętych bez pasma roku (`selection_counts`).
2. **`reserve` = kolejne tytuły po zebraniu całego `main`.** Zbieracz pracuje dalej do zatrzymania przez właściciela (planowany powrót około 27.09), pierwotnego czterodobowego deadline albo wyczerpania katalogu. Najpierw spin-offy, następnie OVA/SPECIAL, długie serie, potem pozostałe wpisy; wewnątrz priorytetów ten sam dobór według lat, popularności i ziarna. Deduplikacja obejmuje cały `main` i `reserve`. Przed zbieraniem rezerwy kończy się paginacja katalogu, więc możliwa jest przerwa w przyroście rekordów. Rezerwa jest świeżym materiałem do przyszłych testów lub nowego egzaminu po porażce; **nie uczestniczy w budowie ani strojeniu heurystyki**.
3. **Z-1:** limit łączny 4000 zostaje zniesiony dla zbierania `main` i `reserve`; **≤3800 prób Torrentio w każdym ruchomym oknie 24 h** pozostaje. Tempo adaptacyjne, cooldowny AniList, trwałe liczenie wszystkich prób oraz natychmiastowy stop po 403/Cloudflare pozostają. Pierwotny deadline i historia nie są resetowane. Migracja kolektora schema 3 → 4 wymaga `-AcceptMainReserveContract -PreviousScriptSha256` z dokładnym poprzednim SHA; jest dopisywana do journalu. Nie autoryzuje retry żadnej blokady.
4. **Ocena:** najpierw wszystkie rekordy `main`, także dodatkowe długie serie i ostatnia niepełna paczka po `main_complete`; dopiero po ich rozliczeniu `reserve` w tym samym procesie ciągłym. Dotychczasowe ID paczek, checkpointy i oceny zostają. Migracja journala ocen `continuous-v1` → `continuous-main-reserve-v2` wymaga tych samych jawnych flag ze starym SHA skryptu oceny. Nowe zdarzenia mają `corpus`; addytywne zdarzenia membership przypisują historyczne rekordy do `main` bez nadpisywania ocen.

**Układ i status:** `titles.json` pozostaje nietknięty; `main-membership.json` opisuje pełny korpus główny i przypisanie historycznych publikacji. Rezerwa ma osobne `reserve/membership.json`, źródła i publikacje. Nowe targety i rekordy leżą w `main/` albo `reserve/`, z polem `corpus`; wspólny `manifest.json` odsyła do membership przez SHA-256. Status obu launcherów pokazuje `corpora.main` i `corpora.reserve`, każde z `titles`, `records`, `evaluated` (ostatecznie rozliczone rekordy, nie pojedyncze odpowiedzi modelu). Nie powstaje dodatkowy podgląd dla właściciela.

**Interpretacja historycznych wyliczeń poniżej:** rachunek 3806 zapytań i zapasu do 4000 dotyczy wyłącznie starego podstawowego doboru. Nie stanowi już limitu ani warunku zatrzymania; obowiązują rolling 3800, pierwotne 4 doby i reguły blokad powyżej. Minima i pokrycie fazy 2 dotyczą wyłącznie `main`, nie sumy z rezerwą.

**Aktualizacja oceny — decyzja 2026-09-25:** konfiguracja lokalnego skryptu ma jawne `LlmConfig.timeout_s=900`; A zachowuje 100 rekordów roboczych + 10 kontroli, B używa 50 + 10, budżet obu nadal 32000. Timeout oznacza ponowienie tej samej wielkości, po dwóch timeoutach podział paczki na pół; dla pojedynczego rekordu dalszy podział jest niemożliwy i wymaga decyzji właściciela. Status pokazuje pomyłki A/B każdej kontroli. Pomyłka obu modeli albo trzy pomyłki jednego kierują kontrolę na listę audytu właściciela w `evidence/e1/corpus/` i wyłączają ją z rotacji. Mniej niż pięć pozytywnych lub pięć negatywnych dostępnych kontroli zatrzymuje ocenę. Te ustalenia zastępują poniższe historyczne wspólne rozmiary paczek i regułę dzielenia wyłącznie po ucięciu. Wynik wdrożenia i bramkę zmiany SHA opisuje `outcomes/e1.md`; sama zgoda na retry nie jest migracją kontraktu.
...
- **Obsługa lokalna:** przed startem opiekun przygotowuje archiwum i plik ID-only `%USERPROFILE%/acquisition-corpus/owner-ids.json`. Komendy z roota repo:
  ...
  # Approved upgrade and retry of the diagnosed UA block; use script_sha256 from status
  powershell.exe -NoProfile -File ".\scripts\tmp\acquisition_collect_local.ps1" -Action launch -AcceptAdaptiveContract -PreviousScriptSha256 "<previous-script-sha256>" -RetryBlocked
  # Continuous labeling: detached start/resume, safe status, cooperative stop
  powershell.exe -NoProfile -File ".\scripts\tmp\corpus_labels_local.ps1" -Action launch
  powershell.exe -NoProfile -File ".\scripts\tmp\corpus_labels_local.ps1" -Action status
  powershell.exe -NoProfile -File ".\scripts\tmp\corpus_labels_local.ps1" -Action stop
  ```

  Zwykłe wznowienie nie resetuje liczników/deadline i nie wysyła zapisanych odpowiedzi ponownie. Niepewna próba domyślnie zatrzymuje pracę odwracalną pauzą; jawne `-RetryUncertain` (CLI: `--retry-uncertain`) zapisuje zgodę właściciela w journalu i dopuszcza nową próbę obciążającą budżet, zachowując wcześniejszą. Zgoda nie obejmuje kolejnej awarii nowej próby. Stop jest kooperacyjny, więc trwający HTTP może najpierw się zakończyć. Mismatch SHA/kontraktu wymaga osobnej decyzji; switch retry go nie obchodzi. Wyłącznie wskazana wyżej migracja adaptacyjna ma osobną zgodę i switch.
- **Wykonalność i Z-1:** zgoda [../README.md](../README.md) §4, uzupełniona decyzją z 2026-09-25 powyżej, obejmuje 2000 tytułów, ≤4000 prób Torrentio łącznie, ≤3800 w każdym rolling 24 h i twarde 4 doby lokalnie oraz osobny 7-dniowy N-01. Dla 200 filmów i 1800 pozostałych wpisów maksimum podstawowego doboru to `200 × 1 + 1800 × 2 = 3800` zapytań, przed odjęciem braków mapowania i wspólnych pierwszych/ostatnich odcinków. Próba N-04 ma `3 × 2 = 6` zapytań; nawet bez ponownego użycia jej odpowiedzi razem to 3806, pozostaje `4000 − 3806 = 194` na uzupełnienia lub ponowienia. Tempo jest adaptacyjne: start 5 s, połowienie po 50 czystych 200 JSON do minimum 1 s, 429 podwaja odstęp i zachowuje cooldown oraz trwały floor. Próba 3801 musi zaczekać na zwolnienie miejsca w rolling 24 h. Katalog, opóźnienia i przestoje mogą wyczerpać okno 4 dób; wynik kończy się wtedy jawną luką i eskalacją, bez automatycznego wydłużenia i bez deklaracji stałego ETA. 3806 jest szacunkiem górnym podstawowego doboru, nie wymaganym minimum ani warunkiem preflight — rzeczywisty dobór często potrzebuje mniej prób. Używać trwałego licznika wszystkich prób, także błędnych i niepewnych, zapisywanego przed transportem. 4000 jest granicą wymagającą nowej zgody przed przekroczeniem, nie deklarowanym limitem serwisu.
...
- **Ocena poprawności (etykiety) — cały korpus, nie próbka** (decyzja właściciela 2026-09-23: nieograniczona liczba agentów OpenCode):
   - **Tryb ciągły — decyzja 2026-09-25:** oznaczanie działa równolegle ze zbieraczem, odczytując co 180 s wyłącznie kompletne publikacje `title` z zakończonych linii journalu oraz pliki i źródła o zgodnych SHA-256. Paczki po 100, końcowa niepełna dopiero po terminalnym zakończeniu zbieracza; nadal 5 pozytywnych + 5 negatywnych kontroli i 32000. Pary A/B i deterministyczne części zachowują trwałe ID; jeden pisarz JSONL odtwarza ukończone oceny, dyskusje i podziały bez ponownego wysyłania zapisanych wyników. Koniec następuje dopiero po terminalnym stanie zbieracza i rozliczeniu całej dostępnej puli, także gdy zbieracz zakończył się luką lub blokadą. Pauza/cooldown nie jest końcem. Checkpointy leżą osobno poza checkoutem, domyślnie `%USERPROFILE%/acquisition-labels/`; bezpieczne podsumowanie i lista właściciela (wszystkie spory po dyskusji, zgodne nieustalone, odrzucone oceny oraz 50 losowych zgodnych, seed 20260925) w lokalnym `evidence/e1/corpus/`, w częściach <500 KiB. Mniej niż 50 dostępnych zgodnych jest jawną luką audytu. Lista zawiera ID i powód skierowania; wejścia i kapsuły odczytuje opiekun z chronionych źródeł. Przed fazą 2 podlega granicy dostępu z sekcji podziału.
   - **Konto 2 — aktualny kontrakt D2 z outcomes/e1.md:** `Settings.palantir_fallback_token` i `UserSettings.palantir_fallback_enrollment_base_url` trafiają przez `load_model_catalog` → `palantir_llm_config` do istniejącego silnika; zastępują historyczne nazwy pól w dalszym opisie fazy 0. Konfiguracja jest odświeżana przy każdym starcie oraz na granicach grup po co najmniej N zakończonych paczkach (domyślnie N=10, parametr `-RefreshBatches`), bez zmiany konfiguracji trwających wywołań. Brak kompletnego konta 2 jest dozwolony i daje wyłącznie komunikat „konto 2: nieustawione”; metryki obecności to booleany. Retry/failover należą do `LlmService`; reguła globalnej współbieżności 4/+2/połowa poniżej pozostaje w mocy. Budżet zawsze 32000; dzielenie tylko po ucięciu, a trwały 4xx zatrzymuje, zgodnie z rozstrzygnięciem outcomes/e1.md.
...
- **Odstępstwo zatwierdzone przez orkiestratora — 2026-09-25:** próbę wykonujemy przed fazą 2, na 3 kandydatach z pewną etykietą `match` w `tests/fixtures/acquisition/identity-231.json` (1 pojedynczy plik, 2 paczki), zamiast czekać na `zgodnych` z N-02. To największa niewiadoma masterplanu; etykiety mają ręcznie zweryfikowane kotwice, a historyczne 231 jest jawnie częścią roboczą według sekcji „Podział”, więc nie ujawnia części odłożonej. Hash bez metadanych w 10 min oznacza wynik nierozstrzygnięty, nie porażkę mechanizmu; wtedy próbujemy najwyżej 2 zapasowych kandydatów na klasę. Bez kompletu 3 rozstrzygających obserwacji nadal obowiązuje stop przed E2 i decyzja właściciela. Wynik, także puste pliki utworzone przy ustawianiu priorytetów, zapisano w `outcomes/e1.md`, sekcja N-03.
...
**Torrentio — wymaganie transportu z 2026-09-25:** adapter zawsze wysyła `User-Agent` AniShift. Sprawdzono lokalnie: domyślny nagłówek `httpx` powoduje Cloudflare 403, `AniShift/0.1` daje 200. W fazie 4 przenieść istniejącą `nyaa.py:USER_AGENT` do wspólnego `anishift/services/http_requests.py:USER_AGENT` i importować ją w obu adapterach Nyaa/Torrentio, bez kopii wartości. Zbieracz fazy 1 korzysta do tego czasu z importu z Nyaa. Zmiana aplikacji należy dopiero do fazy 4.
...
**Pliki:** `scripts/tmp/acquisition_probe.py` (CREATE, surowy zbieracz `collect` → `movies`/`corpus` bez importu przyszłej domeny i adapterów), `acquisition_collect_local.ps1` i `corpus_labels_local.ps1` (odłączone launchery Windows), `corpus_labels.py`, `corpus_labels_prompt.md`, `corpus_labels_debate_prompt.md`; dane według §9 N-02. Zbieranie i przegląd etykiet prowadzi opiekun danych, nie przyszły autor heurystyki.

**Działania — aktualizacja 2026-09-25:** potwierdzić licznik i okno Z-1 z §9 N-02: 3800 prób Torrentio/rolling 24 h, tempo adaptacyjne, bez limitu łącznego 4000, pierwotne 4 doby. Zachować oryginalne 2000 tytułów, dodać warstwową pulę 50 długich serii do `main`, następnie zbierać osobną rezerwę według priorytetów z §9. Przy migracji zachować wszystkie archiwa, liczniki i oceny. Równolegle oznaczać najpierw cały `main`, potem `reserve`, niezależnie przez A i B, z kontrolami i dyskusją D3: świeże instancje Astra/Opus 5, anonimowa losowana kolejność ocen, najwyższe rozumowanie, 1–3 rundy, konkretny fakt i argument partnera; kontrole poza dyskusją. Stosować regułę współbieżności 4/+2/połowa. Konto 2 obsługiwane, ale niewymagane. Jeden pisarz zapisuje membership, kapsuły, etykiety i liczbę rund, bez pełnych odpowiedzi. Właściciel rozlicza brak zgody/nieustalone oraz 50 losowych zgodnych ocen. Zachować pochodzenie i siłę etykiet oraz braki źródeł. Status obu launcherów rozdziela tytuły, rekordy i ocenione `main`/`reserve`.

**Szczegóły działań — decyzja 2026-09-25:** **3800/rolling 24 h** i adaptacja **5 s → połowa po 50 czystych 200 JSON → minimum 1 s; 429 → ×2 + cooldown i trwały floor; 403/Cloudflare/challenge → stop bez retry**. Limity 4000 i 4 dób oraz historia pozostają w mocy. Każde HTTP zbieracza używa `USER_AGENT` z Nyaa; wznowienie znanego lokalnego 403 wymaga jawnych switchy migracji i retry z §9. Uruchomić równolegle tryb `continuous` etykieciarza przez drugi launcher, z kompletnymi publikacjami, odświeżaniem konta 2 i automatycznym zakończeniem po rozliczeniu terminalnego zbieracza. Zbieracz i labeler mają osobne blokady pojedynczego pisarza; status odczytuje rzeczywiste zajęcie blokady obok zapisanego stanu, bez komend procesów. Po rozliczeniu puli `source_complete` daje status `complete`, a `source_stopped` daje `partial`, także dla zera rekordów; `pool_drained` potwierdza wyłącznie rozliczenie dostępnej puli. Jawne `-RetryEvaluatorBlocked` (`--retry-evaluator-blocked`) po konkretnej diagnozie i usunięciu przyczyny przez właściciela autoryzuje dopisaniem tylko istniejące błędy usługi `blocked` A/B lub dyskusji. Nie obejmuje `blocked_minimum`, ucięcia, zmiany kontraktu ani następnej awarii; poprawne oceny zostają. Samo dodanie switcha nie upoważnia do jego użycia live. Kontrole offline: przyspieszanie, reset serii czystych odpowiedzi, 429/floor/cooldown, 403/challenge, wszystkie nagłówki UA, budżet/resume/migracja; fake service dla przyrostu, idempotencji, końca, konta 2 i resume. Pełne bramki i operacje live wykonuje orkiestrator po review.
...
**Gate — aktualizacja 2026-09-25:** spełnione minima i pokrycie warstw `main` w granicach zgody. Brak danych po pierwotnych 4 dobach → pytanie Z-1 w §15; 4000 prób nie zatrzymuje zbieracza. Nie przechodzić do budowy na mniejszym korpusie ani automatycznie wydłużać okna. Rezerwa nie uzupełnia liczności ani brakujących warstw głównego korpusu. N-04 zachowuje swoją regułę decyzji; brak kandydatów filmowych zapisuje lukę do rozstrzygnięcia.
...
**Granica danych — decyzja 2026-09-25:** podział, budowa i pierwszy egzamin obejmują wyłącznie `main` (2000 + dodatkowa pula długich serii). `reserve` pozostaje oddzielną, niewykorzystaną pulą dla przyszłych testów lub nowego egzaminu po porażce. Samo zebranie i oznaczenie rezerwy nie daje autorowi heurystyki dostępu do jej rekordów. Nie wykonywać fazy 2 w ramach wdrażania zbieracza/rezerwy.
...
2a. **Decyzja 2026-09-25:** jawny `User-Agent` AniShift dla każdego żądania Torrentio; wspólna stała w `anishift/services/http_requests.py` przeniesiona z Nyaa, importowana przez oba adaptery (§10.1). Test transportu sprawdza nagłówek również przy ponowieniu. Domyślny UA `httpx` nie jest dopuszczalny — lokalny dowód Cloudflare 403 wobec 200 z `AniShift/0.1`.
...
**Jedno pytanie do właściciela — rozszerzenie Z-1, jeśli podstawowa próba nie wystarczy:** czy zezwalasz na dodatkowy wolumen/czas lokalnie na Windows, gdy 2000 tytułów, maksymalnie 4000 prób Torrentio przy 3800/rolling 24 h, adaptacyjnym tempie i twardych 4 dobach nie da ≥ 10 000 unikalnych oznaczonych rekordów we wszystkich warstwach albo zabraknie świeżej części do kolejnego egzaminu? Podstawa to Z-1 uzupełniona decyzją z 2026-09-25 (§9). Podstawowy dobór to najwyżej 3800 zapytań + 6 N-04 = 3806 (194 rezerwy). Czas zależy od adaptacji, ruchomego budżetu, cooldownów i pozostałych źródeł; przestoje mogą zakończyć próbę luką, bez automatycznego wydłużenia. Wymagana średnia ≥ 2,632 rekordu na podstawowe zapytanie; przy 4000 całkowitych próbach dolna granica to 2,5. Nie da się dziś podać uczciwego dodatkowego wolumenu: przy zatrzymaniu przedstawić rzeczywiste `R` rekordów, brak `max(0, 10000 − R)`, brakujące warstwy/tytuły i zmierzone tempo. Dla dodatniego tempa `y` unikalnych rekordów na próbę orientacyjny dodatkowy wolumen to `ceil(max(0, 10000 − R) / y)`, liczba pojemności dobowych `ceil(dodatkowe_próby / 3800)`; nie jest to ETA, bo istniejące próby w rolling 24 h nadal zajmują budżet. Pokrycie warstw i świeży egzamin wycenić osobno z danych, bez obietnicy wynikającej tylko ze średniej. Gdy `y=0`, raportować brak podstaw do oszacowania. Bez zgody nie przekraczać wolumenu, liczby tytułów ani okna; 7 dni N-01 nie jest zgodą na 7 dni dodatkowego korpusu. To pytanie warunkowe: obecna zgoda wystarcza do podstawowego zbierania, lecz nie gwarantuje osiągnięcia nowego celu. Wyniki N-01–N-04 pozostają zadaniem E1 według §9.
