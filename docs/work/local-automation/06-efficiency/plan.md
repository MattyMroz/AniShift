...
- Częściowy wynik: zastąpiono `IDI_APPLICATION` oryginalną maskotką w ICO z usuniętymi przezroczystymi marginesami. Wygenerowany alternatywny obraz został odrzucony i nie jest częścią aplikacji. Dodano obsługę kliknięcia myszą, klawiaturą i powiadomienia AniShift w protokole v4; natywny test sprawdza własny uchwyt ikony i przekazanie akcji otwarcia. Rozmiar miejsca na ikonę wyznacza Windows; nie deklarować dwukrotnego powiększenia całej sylwetki ani odbioru wizualnego.
- Właściciel zaakceptował ikonę i polecił commit. Kliknięcie ma otwierać ekran startowy, nie Stan; obecne `--state` oraz wymuszenie Stanu w otwartym panelu wymagają korekty. Usunąć również ostrzeżenie `runpy` wynikające z importu `cli.main` w `cli/__init__.py` przed startem przez `-m`.
...
- Powiadomienie AniShift „Gotowy odcinek” dotyczy przetwarzania i otwiera panel. Zgłoszenie nieklikalnego powiadomienia o pobraniu wymaga potwierdzenia emitenta; samo przekazanie `NIN_BALLOONUSERCLICK` nie jest dowodem otwierania GUI qBittorrenta.
- Naprawiono mylne klasyfikowanie ręcznego przejęcia qBittorrenta jako awarii startu. Odczyt sprawdzonego prywatnego GUI działa bez przejmowania sterowania; po jego zamknięciu pobieranie pozostaje zatrzymane. Panel pokazuje po polsku rozróżnienie tych stanów. Nie wznowiono rzeczywistych pobrań.
