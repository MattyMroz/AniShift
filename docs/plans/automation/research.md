...
| `torrents/files` zwraca `index`, `name`, `size`, `progress`, `priority`, `availability`, `piece_range`, `is_seed`; brak pola atrybutów/symlinku | jw.; wiki 5.0 potwierdza brak | verified poza `is_seed` |
| Wydanie 5.2.3 zwraca `is_seed` wyłącznie przy indeksie 0, a pozostałe wpisy nie mają tego klucza; wartość jest stanem całego torrenta, nie dowodem kompletności pliku — kompletność pliku dowodzi `progress == 1.0`, liczone z kawałków po weryfikacji hasha | pomiar na zbudowanym qBittorrencie 5.2.3, Web API 2.15.1, 2026-09-16 | verified (pomiar wydania; master nie dowodzi kontraktu 5.x) |
